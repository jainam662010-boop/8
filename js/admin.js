          ...
          <div class="gcard" style="padding:14px">
            ...
            <div class="fg2">
              <div class="fg"><label>Content Type</label>
                <select class="fc" id="l-type">
                  <option value="video">🎬 Video</option>
                  <option value="explanation">🧠 Explanation</option>
                  <option value="one-shot">⚡ One-shot</option>
                </select>
              </div>
              <div class="fg"><label>Teacher (optional)</label>
                <select class="fc" id="l-teacher">
                  <option value="all">All Teachers</option>
                  ${allTeach.map(t=>`<option value="${encodeURIComponent(t.id)}">${VidyaSec.sanitize(t.name)}${t.channel?` — ${VidyaSec.sanitize(t.channel)}`:''}</option>`).join('')}
                </select>
              </div>
            </div>
            <div class="fg2">
...
DIFF: easy">${this._getQuizRaw(ov)}</textarea>
  ...
  editLesson(sid,cid,idx){
    ...
    App.loadData().then(()=>{
      ...
      const lt=document.getElementById('l-type'); if(lt) lt.value=(l.type||'video');
      const tch=document.getElementById('l-teacher'); if(tch) tch.value=(l.teacher||'all')==='all'?'all':encodeURIComponent(l.teacher||'all');
      document.getElementById('lesson-form-title').textContent=`✏️ Editing Lesson ${idx+1}`;
    });
  },
  cancelEditLesson(){
    this._editingLesson=null;
    if(document.getElementById('lesson-form-title'))document.getElementById('lesson-form-title').textContent='➕ Add New Lesson';
    ['l-url','l-title','l-dur','l-id'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
    const lt=document.getElementById('l-type'); if(lt) lt.value='video';
    const tch=document.getElementById('l-teacher'); if(tch) tch.value='all';
  },
  ...
  saveLesson(sid,cid){
    ...
    const type=VidyaSec.clampStr(document.getElementById('l-type')?.value||'video',20) || 'video';
    const teacherRaw=VidyaSec.clampStr(document.getElementById('l-teacher')?.value||'all',180) || 'all';
    const teacher = teacherRaw==='all' ? 'all' : decodeURIComponent(teacherRaw);
    ...
    App.loadData().then(()=>{
      ...
      const newLesson={id:lid||`l${lessons.length+1}`,title,videoId,duration:dur,type,teacher};
